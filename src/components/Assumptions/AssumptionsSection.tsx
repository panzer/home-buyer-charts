...
const AssumptionSection: React.FC<{ subtitle: string, rows: ItemRowType[] }> = ({ subtitle, rows }) => {
    return (
      <Grid
        size={{ 
            xs: 12,
        ...
        padding={1}
      >
        <Grid size={"grow"}><Typography variant="subtitle1">{subtitle}</Typography></Grid>
        
        <AlternatingRowsContainer container size="grow" columns={6}>
          {rows.map((itemRow, index) => <ItemRow {...itemRow} key={index} />)}
